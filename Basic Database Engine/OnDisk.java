...
public class OnDisk {
    ...
    public static HashMap<String,List<String>> table_columns = new HashMap<>();
...
//                System.out.println(ddl);
                ...
                if(query instanceof CreateTable){
                    ...
                    buildIndex(ct.getTable().getName(),indexColumns);
    ...
    public static void buildIndex(String tableName, List<String> indexColumns) throws IOException {
        /*** LINEITEM ***/
        if(tableName.equals("LINEITEM")) {
            for (int index = 0; index < indexColumns.size(); index++) {
                List<OrderByElement> orderBy = new ArrayList<>();
                Column c = new Column();
                c.setColumnName(indexColumns.get(index).split("\\.")[1]);
                c.setTable(new Table(indexColumns.get(index).split("\\.")[0]));
                OrderByElement o1 = new OrderByElement();
                o1.setExpression(c);
                orderBy.add(o1);
                buildTwoAttributeIndex(tableName, orderBy);

                for (int i = 1, j = 1; i <= 6; i = i + 2, j++)
                    merge("/home/shruti/DB/Checkpoint2_on_disk/data/" + indexColumns.get(index) + "_S" + i + ".csv", "/home/shruti/DB/Checkpoint2_on_disk/data/" + indexColumns.get(index) + "_S" + (i + 1) + ".csv", 0, indexColumns.get(index), j, orderBy);
                merge("/home/shruti/DB/Checkpoint2_on_disk/data/" + indexColumns.get(index) + "_M01.csv", "/home/shruti/DB/Checkpoint2_on_disk/data/" + indexColumns.get(index) + "_M02.csv", 1, indexColumns.get(index), 1, orderBy);
                merge("/home/shruti/DB/Checkpoint2_on_disk/data/" + indexColumns.get(index) + "_M03.csv", "/home/shruti/DB/Checkpoint2_on_disk/data/" + indexColumns.get(index) + "_M11.csv", 2, indexColumns.get(index), 1, orderBy);
        ...
        /*** rest of the tables ***/
            ...
            for (int index = 0; index < indexColumns.size(); index++) {
                ...
                BufferedReader file_br = new BufferedReader(new FileReader("/home/shruti/DB/Checkpoint2_on_disk/data/" + tableName + ".csv"));
                String line = file_br.readLine();
                while (line != null) {
                    data.add(new StringBuilder(line));
                    line = file_br.readLine();
                }
                MyComparator_1 compare = new MyComparator_1(schema.get(tableName.split("\\.")[0]), orderBy);
                Collections.sort(data, compare);

                BufferedWriter file_bw = new BufferedWriter(new FileWriter("/home/shruti/DB/Checkpoint2_on_disk/data/" + ((Column)orderBy.get(0).getExpression()).getTable().getName()+"."+ ((Column)orderBy.get(0).getExpression()).getColumnName() + "_M21.csv", true));
                System.out.println(data.size());
                for (int i = 0; i < data.size(); i++) {
                    file_bw.write(data.get(i).delete(data.get(i).length() - 2, data.get(i).length()).toString());
                    file_bw.newLine();
                }
                file_bw.close();
            }
    ...
    public static void putRemainingLines(String currentLine, BufferedReader bufferedReader, BufferedWriter bufferedWriter){
    ...
    public static void merge(String file1, String file2,int level,String tableName,int number,List<OrderByElement> orderBy){

        try {
            File file = new File("/home/shruti/DB/Checkpoint2_on_disk/data/"+tableName+"_M"+level+""+number+".csv");
            if(!file.exists()){
                file.createNewFile();
            }
            ...
            while(line!=null && line1!=null) {
                MyComparator_1 compare = new MyComparator_1(schema.get(tableName.split("\\.")[0]),orderBy);
                if(compare.compare(new StringBuilder(line),new StringBuilder(line1)) ==0){
                    ...
                    if(line==null){
                        putRemainingLines(line1, bufferedReader1, bufferedWriter);
                    }
                    else if(line1==null){
                        putRemainingLines(line, bufferedReader, bufferedWriter);
                    }
                }
                else if(compare.compare(new StringBuilder(line),new StringBuilder(line1)) >0){
                    bufferedWriter.write(line1);
                    bufferedWriter.newLine();
                    line1 = bufferedReader1.readLine();
                    if(line1==null){
    ...
    public static void buildTwoAttributeIndex(String tableName, List<OrderByElement> orderBy) throws IOException {
        ...
        for(int loop =1;loop<=6;loop++) {
            ...
            while (count < 100100*loop && line != null) {
                line = file_br.readLine();
                if(line==null)
                    break;
                data.add(new StringBuilder(line));
            ...
            FileWriter i_fileWriter = new FileWriter("/home/shruti/DB/Checkpoint2_on_disk/data/" + ((Column)orderBy.get(0).getExpression()).getTable().getName()+"."+ ((Column)orderBy.get(0).getExpression()).getColumnName() + "_S"+loop+".csv", true);
            BufferedWriter file_bw = new BufferedWriter(i_fileWriter);
            System.out.println(data.size());
            for (int i = 0; i < data.size(); i++) {
                file_bw.write(data.get(i).delete(data.get(i).length() - 2, data.get(i).length()).toString());
                file_bw.newLine();
    ...
    public static HashMap<String,DBschema> buildSchema(ArrayList<String> joinTables) {
        String basetable = joinTables.get(0);
//        joinTables.remove(0);
        ...
        for(int j=1;j<joinTables.size();j++) {
...
//                    alreadyJoinedTables.remove(0);
                    /*** build new schema ***/
                    HashMap<String,DBschema> new_schema = buildSchema(alreadyJoinedTables);
                    ...
                    //constructing list of files-first argument
                    ArrayList<String> joinFilesList = new ArrayList<String>();
                    HashMap<Integer,HashMap<String,DBschema>> schemas = new  HashMap<Integer,HashMap<String,DBschema>>();
                    ArrayList<List<String>> alljoinTableColumns = new  ArrayList<List<String>>();
                    int joinColumnNum = 0;
                    int schemaNo = 0;


                    for(int i =0; i<alreadyJoinedTables.size();i++){
                        String currTable = alreadyJoinedTables.get(i);
                        String joinColumnName = orderedColumnsWithTableNames.get(joinColumnNum);
                        if(joinColumnNum == 0 && i==0){
                            String firstJoinColumnName = joinColumnName.split("\\.")[1];
                            joinFilesList.add("/home/shruti/DB/Checkpoint2_on_disk/data/"+currTable+"."+firstJoinColumnName+"_M21.csv");
                        }else{
                            joinFilesList.add("/home/shruti/DB/Checkpoint2_on_disk/data/"+joinColumnName+"_M21.csv");
                            joinColumnNum++;
                        }
                    }
                    //constructing hashmap of schemas and list of all table columns

                    for(String table: alreadyJoinedTables){
                        schemas.put(schemaNo, schema.get(table));
                        alljoinTableColumns.add(table_columns.get(table));
                        schemaNo++;
                    }
...
//                    List<Expression> iter_where = buildWhereExpressionList(normalWhereExpressionList,baseTable);
                    ...
                    myIter = new MyIterator(joinFilesList,schemas,fetchIndicesForJoin,orderedColumnsWithTableNames,alljoinTableColumns);
//                    myIter = new MyIterator(joinTableIndex,joinTableData,baseTable+"_1",table_columns.get(baseTable),schema.get(baseTable),fetchIndicesForJoin,removalIndices,additionalIndex,new_schema,iter_where);
                }
                else
